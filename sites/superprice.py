...
def crawl_superprice(sc_items,process, fuzz):
	try:
	    element = WebDriverWait(browser, 50,5).until(
	        EC.presence_of_element_located((By.CSS_SELECTOR , 'li[class^="item-inner"]'))
	    )
		...
		try:
			...
			products = browser.find_element_by_class_name('product-grid').find_elements_by_class_name('product-item-toki')
			print(len(products))
			for product in products:
				# print(product.get_attribute('innerHTML'))
				name = product.find_element_by_class_name('product-name').get_attribute('innerText').strip()
				url = product.find_element_by_class_name('product-name').find_element_by_tag_name('a').get_attribute('href')
				price = re.sub('[^0-9.]','',product.find_element_by_css_selector('span[class="price"]').get_attribute('innerText').strip())
				image_url = product.find_element_by_css_selector('img[class="product-image-photo"]').get_attribute('src').strip()
	...
	try:
		...
		next_url_page = browser.find_element_by_css_selector('a[title="Next"]').get_attribute('href').strip()
